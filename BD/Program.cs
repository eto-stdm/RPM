...
namespace BD
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                Details client_detail = client_arrives();
                car_fixed = false;
            ...
            void fix_car(Details client_detail)
            {
                Console.WriteLine("********************************");
                ...
                if (c == "1")
                {
                    ...
                    try
                    {
                        ...
                        decimal sum = Convert.ToDecimal(client_detail.Detail_price) + Convert.ToDecimal(1000);
                        foreach (var item in repair) { item.Balance += sum; }
                        Console.WriteLine($"Вы получили {sum} рублей."); // + на счету
                        car_fixed = true;
                        Console.WriteLine("++++++++++++++++++++++++++++++++++++++++++++++++++");
                    }
                    catch // нет детали
                        ...
                        { 
                            if (item.Balance >= buy_detail) // если денег достаточно для покупки
                            {
                                try
                                {
                                    ...
                                    Car_repair_details new_detail = new Car_repair_details
                                    {
                                        Car_repair_id = 1,
            ...
            void check_my_items()
            {
                Core.Context.SaveChanges();
                Console.WriteLine("****************************************");
                Console.WriteLine("Предметы автосервиса:");
